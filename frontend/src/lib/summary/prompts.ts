...
4. 使用 [N] 标记引用来源（N 是参考清单中的编号）

输出格式（严格JSON，不要添加 markdown 代码块标记）：
{
  "quick_scan": [
    { "text": "事件描述，用[N]标记引用来源，例如：OpenAI算力目标达6000亿美元[1]", "source": "来源名称" }
  ],
  ...
  "sentiment": {
    ...
    "controversies": [
      { "topic": "争议主题", "analysis": "各方立场和分歧要点，用[N]标记引用来源" }
    ],
    "opinion_shifts": [
      { "topic": "转向主题", "analysis": "相比近期的舆论方向变化，用[N]标记引用来源" }
    ],
    "risk_flags": [
      { "topic": "风险信号", "analysis": "具体负面信号及潜在影响，用[N]标记引用来源" }
    ]
  }
}
...
板块说明：
- 速览（quick_scan）：挑选最重要的3-5条具体新闻或观点。每条只讲一件事。筛选标准：重大产品发布、关键人物判断、重大融资/并购事件、重要政策变化、市场重大波动。
