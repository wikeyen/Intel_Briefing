# ABOUTME: Legacy external news fetchers using httpx and BeautifulSoup.
# ABOUTME: Provides per-source fetch functions for HN, GitHub, 36Kr, V2EX, WallStreetCN, etc.
import argparse
import json
import logging
import httpx
...
def fetch_github(limit: int = 5, keyword: Optional[str] = None) -> List[Dict]:
    try:
        response = httpx.get("https://github.com/trending", headers=HEADERS, timeout=10, follow_redirects=True)
    except httpx.HTTPError as e:
...
def fetch_36kr(limit: int = 5, keyword: Optional[str] = None) -> List[Dict]:
    try:
        response = httpx.get("https://36kr.com/newsflashes", headers=HEADERS, timeout=10, follow_redirects=True)
        soup = BeautifulSoup(response.text, 'html.parser')
        items = []
        ...
        return filter_items(items, keyword)[:limit]
    except (httpx.HTTPError, AttributeError, KeyError, TypeError) as e:
...
def fetch_v2ex(limit: int = 5, keyword: Optional[str] = None) -> List[Dict]:
    try:
        data = httpx.get("https://www.v2ex.com/api/topics/hot.json", headers=HEADERS, timeout=10, follow_redirects=True).json()
        items = []
        for t in data:
...
def fetch_tencent(limit: int = 5, keyword: Optional[str] = None) -> List[Dict]:
    try:
        ...
        data = httpx.get(url, headers={"Referer": "https://news.qq.com/"}, timeout=10, follow_redirects=True).json()
        items = []
...
def fetch_wallstreetcn(limit: int = 5, keyword: Optional[str] = None) -> List[Dict]:
    try:
        ...
        data = httpx.get(url, timeout=10, follow_redirects=True).json()
        items = []
...
def fetch_producthunt(limit: int = 5, keyword: Optional[str] = None) -> List[Dict]:
    try:
        response = httpx.get("https://www.producthunt.com/feed", headers=HEADERS, timeout=10, follow_redirects=True)
        ...
        return filter_items(items, keyword)[:limit]
    except (httpx.HTTPError, AttributeError, ValueError) as e:
