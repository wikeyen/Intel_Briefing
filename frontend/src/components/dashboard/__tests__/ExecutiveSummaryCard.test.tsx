...
// ABOUTME: Covers null/empty states, mood badge, citations via FormattedSummaryText, quick scan, and collapsible risk flags.
...
describe('ExecutiveSummaryCard', () => {
  ...
  // ---------------------------------------------------------------------------
  // FormattedSummaryText integration (paragraphs, bullets, citations)
  // ---------------------------------------------------------------------------

  it('renders multi-paragraph summary as separate <p> elements', () => {
    const summary = makeSummary({
      executive_summary: 'First paragraph.\n\nSecond paragraph.',
    })
    const { container } = render(createElement(ExecutiveSummaryCard, { summary }))
    const paragraphs = container.querySelectorAll('p')
    // Filter to only the executive summary paragraphs (exclude risk flags etc.)
    const execParagraphs = Array.from(paragraphs).filter(p =>
      p.textContent === 'First paragraph.' || p.textContent === 'Second paragraph.'
    )
    expect(execParagraphs).toHaveLength(2)
  })

  it('renders bullet blocks as <ul>/<li>', () => {
    const summary = makeSummary({
    ...
    expect(listItems[0].textContent).toBe('Bullet one')
    expect(listItems[1].textContent).toBe('Bullet two')
  })
  ...
  it('renders citation links when sources are provided', () => {
    const summary = makeSummary({
      ...
      sources: [
        { id: 1, title: 'Bloomberg Report', url: 'https://bloomberg.com/1', sensor: 'rss_news' },
        { id: 2, title: 'Reuters Update', url: 'https://reuters.com/2', sensor: 'rss_news' },
      ],
    })
    const { container } = render(createElement(ExecutiveSummaryCard, { summary }))
